<?php  
    ...
    $stmt = $conn->prepare("SELECT c.channel_no, d.doctor_name, p.name, c.room_no, c.channel_date FROM channel c JOIN doctor d ON d.doctor_id = c.doctor_no JOIN patient p ON p.patientno = c.patient_no WHERE d.log_id = ?");
